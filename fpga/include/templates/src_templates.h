...
#define SRC_KERNEL_uchar(UNROLL, OUT_CHANNEL) __kernel                              \
void kernel_source(                                                                 \
 ...
 uint items) {                                                                      \
  ...
  while (counter < items) {                                                         \
    ...
    write_channel_intel(OUT_CHANNEL, data);                                         \
    read += (UNROLL);                                                               \
    if (read == 32) read = 0;                                                       \
    counter += (UNROLL);                                                            \
  }                                                                                 \
...
#define SRC_KERNEL_uchar_1(UNROLL, OUT_CHANNEL) __kernel                            \
void kernel_source(                                                                 \
 __attribute__((intel_host_accessible, blocking)) __read_only pipe ulong4 host_in,  \
 uint items) {                                                                      \
  int counter = 0;                                                                  \
...
#define SRC_KERNEL_uchar_16(UNROLL, OUT_CHANNEL) __kernel                           \
void kernel_source(                                                                 \
 __attribute__((intel_host_accessible, blocking)) __read_only pipe ulong4 host_in,  \
 uint items) {                                                                      \
  int counter = 0;                                                                  \
...
#define SRC_KERNEL_uchar_32(UNROLL, OUT_CHANNEL) __kernel                           \
void kernel_source(                                                                 \
 __attribute__((intel_host_accessible, blocking)) __read_only pipe ulong4 host_in,  \
 uint items) {                                                                      \
  int counter = 0;                                                                  \
...
#define SRC_KERNEL_uint_(UNROLL, OUT_CHANNEL) __kernel \
...
#define SRC_KERNEL_uint_1(UNROLL, OUT_CHANNEL) __kernel \
void kernel_source( __attribute__((intel_host_accessible, blocking)) __read_only pipe ulong4 host_in, uint items) {\
  int counter = 0;\
  ulong4 data_in;\
  TYPE(data, uint, UNROLL);\
